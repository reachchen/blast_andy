...
public class StaticIpUtil {
    Context mContext;
    private final String dns1 = "8.8.8.8";
    private final String dns2 = "8.8.4.4";
    ...
    public void getNetworkInformation() {
        ...
        String gateway = long2ip(getwayIpS);
        // 注意要加\\,要不出不来,yeah
        String[] IPS = IP.split("\\.");
        IP = IPS[0] + "." + IPS[1] + "." + IPS[2] + "." + Constant.IPLast;
        ...
        if (!Constant.IP.equals(zeroIP) && !Constant.IP.equals(equalIP)) {
            setIpWithTfiStaticIp(false, IP, Constant.prefix, dns1, gateway);
        }
    }*/
